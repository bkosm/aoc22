...
fun positionOfSignalOfSize(line: String, signalSize: Int): Int {
    val indexOfFirstUniqueSet = line.toList().windowed(signalSize, 1).mapIndexed { i, it ->
        if (areAllUnique(it)) i
        else null
    }.filterNotNull().first()

    return indexOfFirstUniqueSet + signalSize
}
